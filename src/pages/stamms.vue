	...
	<v-responsive class="border rounded">
		<v-app>
			<v-main class="flex-grow-1">
				<v-container fluid class="d-flex flex-column align-center" :class="{ 'blur-background': showForm }" style="max-width: 1600px; margin: auto;">
					<v-card class="elevation-12 rounded-lg" style="width: 100%;">
						...
						<v-card-text class="pa-6">
									...
									<v-form ref="form" @submit.prevent="saveStrainData">
										...
										<v-card-text>
											<v-container>
												<v-row>
													<v-col cols="12" md="12">
														<v-text-field v-model="title" label="Название штамма *" required :rules="[rules.required]" variant="outlined"></v-text-field>
													</v-col>
													<v-col cols="12" md="12">
														<v-autocomplete v-model="location" label="Место выделения *" required :items="countries" :search-input.sync="search" :rules="[rules.required]" variant="outlined"></v-autocomplete>
													</v-col>
													<v-row justify="center" class="w-100">
														<v-col cols="12" md="6" class="d-flex justify-center">
															<v-select variant="outlined" v-model="year_of_allocation" :items="years" label="Год выделения *" required :rules="[rules.required]"></v-select>
														</v-col>
													</v-row>
													<v-col cols="12" md="12">
														<v-card class="dashed-border pa-0" variant="outlined">
															<div class="d-flex justify-center">
																<v-file-input v-model="file" label="Штамм *" accept=".txt" required :rules="[rules.required]" variant="solo" prepend-icon="" class="file-input-width"></v-file-input>
															</div>
							...
							<v-data-table :headers="headers" :items="items" :search="search" :items-per-page="5" :pagination.sync="pagination">
								<template v-slot:top>
		...
		data() {
				return {
					...
					headers: [{
						...
						title: 'Место выделения',
						value: 'place_of_allocation'
					}, {
						title: 'Год выделения',
						value: 'year_of_allocation'
					}, ],
					items: [],
					pagination: {
						page: 1,
