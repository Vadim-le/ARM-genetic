<template>
	<v-app>
		<v-main class="d-flex align-top justify-center" style="height: 100vh; background-color:#1867c0;">
			<v-container>
								...
								<v-form v-if="isLogin">
									<template v-if="tab === 'option-1'">
										...
										<template v-if="tab_inside === 'main'">
											...
											<v-text-field v-model="name" variant="outlined" placeholder="Введите имя" density="compact" dense></v-text-field>
											<div class="text-subtitle-1 text-large-emphasis">Фамилия</div>
											<v-text-field v-model="surname" variant="outlined" placeholder="Введите фамилию" density="compact"></v-text-field>
											<div class="text-subtitle-1 text-large-emphasis">Отчество</div>
											<v-text-field v-model="patronymic" variant="outlined" placeholder="Введите отчество" density="compact"></v-text-field>
											<div class="text-subtitle-1 text-large-emphasis">Пол</div>
											<v-radio-group v-model="inline" inline>
												<v-radio label="Мужской" value="radio-1"></v-radio>
												<v-radio label="Женский" value="radio-2"></v-radio>
											</v-radio-group>
											<v-divider class="border-opacity-100"></v-divider>
											<v-card-title class="mt-n1 ml-n4">Почта</v-card-title>
											<div>
												<div class="d-flex align-center">
													<v-text-field v-model="email" readonly density="compact" variant="plain" outlined></v-text-field>
												...
												<v-dialog v-model="showDialog" max-width="800px">
													<v-card>
														...
														<v-card-text>
															...
															<v-text-field v-model="newPassword" :append-inner-icon="visible ?  'mdi-eye' : 'mdi-eye-off'" :type="visible ? 'text' : 'password'" placeholder="Введите пароль" density="compact" variant="outlined" @click:append-inner="visible = !visible"></v-text-field>
															...
															<v-text-field v-model="name" variant="outlined" placeholder="Введите новую почту" density="compact" dense></v-text-field>
											...
											<v-dialog v-model="showDialogDelete" max-width="800px">
												<v-card>
													...
													<v-card-text>
														...
														<v-text-field v-model="name" variant="outlined" placeholder="Введите фразу" density="compact" dense></v-text-field>
													</v-card-text>
													<v-card-actions>
										...
										<template v-if="tab_inside === 'education'">
											...
											<v-text-field v-model="academicDegree" @input="markAsChanged" variant="outlined" placeholder="Введите ученую степень" density="compact"></v-text-field>

											<div class="text-subtitle-1 text-large-emphasis">Ученое звание</div>
											<v-text-field v-model="academicTitle" @input="markAsChanged" variant="outlined" placeholder="Введите ученое звание" density="compact"></v-text-field>
													...
													<v-card-text>
														<v-row>
															<v-col cols="12">
																...
																<v-text-field v-model="selectedRecord.educational_institute" variant="outlined" @input="markAsChanged" placeholder="Введите учебное заведение" density="compact" dense></v-text-field>
															</v-col>

															<v-col cols="12">
																<div class="text-subtitle-1 text-large-emphasis">Уровень образования</div>
																<v-text-field v-model="selectedRecord.educational_level" variant="outlined" @input="markAsChanged" placeholder="Введите уровень образования" density="compact" dense></v-text-field>
															</v-col>

															<v-col cols="12">
																<div class="text-subtitle-1 text-large-emphasis">Специальность</div>
																<v-text-field v-model="selectedRecord.specialization" variant="outlined" @input="markAsChanged" placeholder="Введите специальность" density="compact" dense></v-text-field>
															</v-col>

															<v-col cols="12">
																<div class="text-subtitle-1 text-large-emphasis">Квалификация</div>
																<v-text-field v-model="selectedRecord.qualification" variant="outlined" @input="markAsChanged" placeholder="Введите квалификацию" density="compact" dense></v-text-field>
										...
										<template v-if="tab_inside === 'contacts'">
											...
											<v-text-field v-model="contact_email" @input="markAsChanged" variant="outlined" placeholder="Введите почту" density="compact"></v-text-field>
													...
													<v-card-text>
														<v-row>
															<v-col cols="12">
																...
																<v-text-field v-model="selectedRecord.journal_title" variant="outlined" @input="markAsChanged" placeholder="Введите название журнала" density="compact" dense></v-text-field>
															</v-col>

															<v-col cols="12">
																<div class="text-subtitle-1 text-large-emphasis">Ссылка на профиль в журнале</div>
																<v-text-field v-model="selectedRecord.journal_link" variant="outlined" @input="markAsChanged" placeholder="Введите ссылку на журнал" density="compact" dense></v-text-field>
																<input type="hidden" v-model="selectedRecord.id" />
															</v-col>
														</v-row>
													</v-card-text>
    ...
    methods: {
      ...
      async fetchData(token) {
        ...
        try {
          ...
          this.name = data.metadata.first_name || '';
          this.surname = data.metadata.last_name || '';
          this.patronymic = data.metadata.patronymic || '';
      ...
      async removeEducationRecord(index) {
        const recordId = this.educationRecords[index].id;
        const token = localStorage.getItem('token');
        try {
      ...
      async removeContactRecord(index) {
        const recordId = this.contactRecords[index].id;
        const token = localStorage.getItem('token');
        try {
